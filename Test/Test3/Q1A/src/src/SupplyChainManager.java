...
class InventoryList {
    ...
    Node searchNode(Product p) {
        Node q = this.head;
        while (q != null) {
            if (q.info.getId().equalsIgnoreCase(p.getId())) {
                return q;
            }
            q = q.next;
        }
    ...
    void addLast(String id, String name, int quantity) {
        if (quantity > 0) {
            Product newP = new Product(id, name, quantity);
...
class ShippingList {
    ...
    void push(String id, int quantity) {
        if (quantity > 0) {
            Product newP = new Product(id, quantity);
            Node newN = new Node(newP);
            newN.next = this.head;
            this.head = newN;
        }
...
class SupplyChainManager {
    ...
    private boolean updateInventory(Product p) {
        Node q = warehouseItems.searchNode(p);
        if (q != null) {
            q.info.setQuantity(q.info.getQuantity() - p.getQuantity());
            return true;
        }
        return false;
    }
    ...
    void f2() throws Exception {
        ...
        Product request = shippingQueue.pop();
        if (request != null) {
    ...
    void f4() throws Exception {
        ...
        while (!shippingQueue.isEmpty()) {
            Product request = shippingQueue.pop();
            if (request != null) {
                boolean check = updateInventory(request);
                Node p = warehouseItems.searchNode(request);
                if(p != null && check){
                    totalShipped += (request.getQuantity());
                }
            }
        }
        // ------------------------------------------------------------------------------------
