...
class BookBST {
    ...
    void insert(String title, int copies, int price) {
        ...
        // --------------------------------------------------------
        Book newB = new Book(title, copies, price);
        this.root = insertRec(this.root, newB);
        // --------------------------------------------------------
    }
...
class BorrowQueue {
    ...
    void enQueue(String title, int copies) {
        ...
        // --------------------------------------------------------
        if (copies > 0) {
            Book newB = new Book(title, copies);
            QueueNode newN = new QueueNode(newB);
            if (isEmpty()) {
...
class LibrarySystem {
    ...
    void f4() throws Exception {
        ...
        while (!(borrowRequests.isEmpty())) {
            Book request = borrowRequests.deQueue();
            if (request != null) {
                Node p = bookCatalog.search(bookCatalog.root, request.getTitle());
                if (p != null && p.info.getCopies() >= request.getCopies()) {
                    totalValue += (request.getCopies() * p.info.getPrice());
                    borrowBook(request);
                }
            }
        }
